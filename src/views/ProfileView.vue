    ...
    <div class="rounded-lg border bg-white px-4 pt-8 pb-10 shadow-lg">
      ...
      <ul class="mt-3 divide-y rounded bg-gray-100 py-2 px-3 text-gray-600 shadow-sm hover:text-gray-700 hover:shadow">
        <li class="flex items-center py-3 text-sm">
          <span>Username</span>
          <span class="ml-auto">{{ getUser.username }}</span> <!-- Menampilkan username pengguna -->
        </li>
        <li class="flex items-center py-3 text-sm">
          <span>Email</span>
          <span class="ml-auto">{{ getUser.email }}</span> <!-- Menampilkan email pengguna -->
        </li>
      </ul>
    </div>
  </div>
...
export default {
  ...
  created() {
    this.fetchMe(); // Memanggil aksi fetchMe untuk mendapatkan data pengguna
  },
};
</script>